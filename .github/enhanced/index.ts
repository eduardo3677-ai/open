...
let enhancedConfig: {
  ...
  configFile: process.env.CONFIG_FILE || "opencode-plus.json",
  enableExtensions: process.env.ENABLE_EXTENSIONS === "true",
  enableRipgrep: process.env.ENABLE_RIPGREP === "true",
...
try {
  ...
  // Apply enhanced configuration extensions
  await applyEnhancedExtensions()
          ...
          try {
            ...
            if (evt.type === "message.part.updated") {
              ...
              if (part.type === "tool" && part.state.status === "completed") {
                ...
                // Enhanced logging with reason (GH-compatible output)
                console.log()
                console.log(`${color}|`, `\x1b[0m\x1b[2m ${tool.padEnd(7, " ")}`, "", `\x1b[0m${title}`)

                // Output more context to GitHub Actions
                if (enhancedConfig.enableExtensions) {
                     core.info(`Tool executed: ${tool}`)
                     core.info(`Input: ${title.substring(0, 200)}`).catch(() => {})
                     if (part.state.output) {
                        core.debug(`Output: ${JSON.stringify(part.state.output).substring(0, 500)}`).catch(() => {})
                     }
                }

                // Show diff when available
                if (part.tool === "edit" && part.state.output?.diff) {
                  console.log()
                  console.log("\x1b[2mDiff:\x1b[0m")
                  console.log(part.state.output.diff)
                  console.log()

                  if (enhancedConfig.enableExtensions) {
                      core.notice("File changed: " + JSON.stringify(part.state.input)).catch(() => {})
                      core.startGroup("Diff Preview").catch(() => {})
                          try {
                              core.info(part.state.output.diff).catch(() => {})
                          } finally {
                              core.endGroup().catch(() => {})
                          }
                  }
              ...
              if (part.type === "text") {
                ...
                if (part.time?.end) {
                  ...
                  // Log reasoning for compact conversation
                  if (enhancedConfig.enableCompression) {
                      core.notice(`Session reasoning: ${text}`).catch(() => {})
                  }
                }
              }
            }
            ...
            if (evt.type === "session.updated") {
              ...
              // Token limit handling with compaction
              if (enhancedConfig.enableCompression && session.usage) {
                  const totalTokens = session.usage.inputTokens + session.usage.outputTokens
                  const maxTokens = session.usage.maxTokens || 100000

                  if (totalTokens > maxTokens * (enhancedConfig.compaction?.triggerTokenRatio || 0.85)) {
                      core.notice(`Token limit approaching (${totalTokens}/${maxTokens}). Compression triggered.`).catch(() => {})
                  }
              }
            }
          } catch {
...
async function loadEnhancedConfiguration() {
  if (!enhancedConfig.enableExtensions) {
    core.info("Enhanced extensions disabled, skipping configuration loading")
    return
  }
  ...
  try {
    const configPath = enhancedConfig.configFile || "opencode-plus.json"
    const configContent = await $`cat ${configPath}`.nothrow()

    if (!configContent.success) {
      console.log(`Configuration file not found: ${configPath}, using defaults`)
      core.warning(`Configuration file not found: ${configPath}`)
      return
    }

    const config = JSON.parse(configContent.stdout.toString())
    enhancedConfig = {
      ...enhancedConfig,
      ...config,
      extensions: {
        ...
        aspServers: config.extensions?.aspServers || [],
        mcpServers: config.extensions?.mcpServers || []
      },
      compaction: {
        auto: config.compaction?.auto ?? true,
        triggerTokenRatio: config.compaction?.triggerTokenRatio ?? 0.85,
        useSmallModel: config.compaction?.useSmallModel ?? true
      }
    }

    // Set log level
    core.info(`Log level set to: ${enhancedConfig.logLevel}`)

    // Log loaded extensions
    if (enhancedConfig.extensions?.plugins?.length) {
      core.info(`Loaded plugins: ${enhancedConfig.extensions.plugins.join(", ")}`)
    }
    if (enhancedConfig.extensions?.skills?.length) {
      core.info(`Loaded skills: ${enhancedConfig.extensions.skills.join(", ")}`)
    }
    if (enhancedConfig.extensions?.aspServers?.length) {
      core.info(`Configured ASP servers: ${enhancedConfig.extensions.aspServers.length}`)
    }
    if (enhancedConfig.extensions?.mcpServers?.length) {
      core.info(`Configured MCP servers: ${enhancedConfig.extensions.mcpServers.length}`)
    }

    console.log(`Enhanced configuration loaded from ${configPath}`)

  } catch (error) {
    console.log(`Error loading configuration: ${error}`)
    core.warning(`Failed to load enhanced configuration: ${error}`)
  }
}

async function applyEnhancedExtensions() {
  if (!enhancedConfig.enableExtensions) {
    return
  }

  console.log("Applying enhanced extensions...")

  try {
    // Install plugins
    if (enhancedConfig.extensions?.plugins?.length) {
      console.log(`Installing ${enhancedConfig.extensions.plugins.length} plugins...`)
      for (const plugin of enhancedConfig.extensions.plugins) {
        console.log(`  - ${plugin}`)
        // Here we would install/load the plugin
        // For now, just log the plugin name
        core.info(`Plugin: ${plugin}`).catch(() => {})
      }
    }

    // Load skills
    if (enhancedConfig.extensions?.skills?.length) {
      console.log(`Loading ${enhancedConfig.extensions.skills.length} skills...`)
      for (const skill of enhancedConfig.extensions.skills) {
        console.log(`  - ${skill}`)
        core.info(`Skill: ${skill}`).catch(() => {})
      }
    }

    // Configure ASP servers
    if (enhancedConfig.extensions?.aspServers?.length) {
      console.log(`Configuring ${enhancedConfig.extensions.aspServers.length} ASP servers...`)
      for (const server of enhancedConfig.extensions.aspServers) {
        console.log(`  - ${server.id}: ${server.name}`)
        core.info(`ASP Server: ${server.id}`).catch(() => {})
      }
    }

    // Configure MCP servers
    if (enhancedConfig.extensions?.mcpServers?.length) {
      console.log(`Configuring ${enhancedConfig.extensions.mcpServers.length} MCP servers...`)
      for (const server of enhancedConfig.extensions.mcpServers) {
        console.log(`  - ${server.id}: ${server.type}`)
        core.info(`MCP Server: ${server.id}`).catch(() => {})
      }
    }

    console.log("Enhanced extensions applied successfully")

  } catch (error) {
    console.log(`Error applying extensions: ${error}`)
    core.warning(`Failed to apply some extensions: ${error}`)
  }
}

async function checkTokenCompressionTrigger() {
  if (!enhancedConfig.enableCompression || !session.usage) {
    return false
  }
  ...
  const totalTokens = session.usage.inputTokens + session.usage.outputTokens
  const maxTokens = session.usage.maxTokens || 100000
  const threshold = enhancedConfig.compaction?.triggerTokenRatio || 0.85

  if (totalTokens > maxTokens * threshold) {
    console.log(`Token limit approaching (${totalTokens}/${maxTokens}), compression may be needed`)
    core.notice(`Token usage: ${totalTokens}/${maxTokens} (${Math.round(totalTokens/maxTokens * 100)}%)`).catch(() => {})
    return true
  }
